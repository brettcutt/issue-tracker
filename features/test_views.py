...
class TestViews(TestCase):
    def setUp(self):
        ...
        self.feature = Features.objects.create(name='feature title',
    ...
    def test_get_feature_detail_page(self):
        response = self.client.get(
            '/features/feature_detail/{0}/'.format(self.feature.id))
        item = get_object_or_404(Features, pk=1)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'feature_detail.html')
        self.assertIn('this is a description', str(response.content))
    ...
    def test_adding_a_new_feature_ticket(self):
        response = self.client.post('/features/add_feature/',
                                    {'name': 'new feature title',
                                     ...
                                     'upvotes': 0,
                                     'status': 'Waiting'})
        item = get_object_or_404(Features, pk=2)
        self.assertEqual(item.name, 'new feature title')
        self.assertEqual(item.description, 'this is a new description')
    ...
    def test_get_edit_feature_page(self):
        response = self.client.get(
            "/features/edit_feature/{0}/".format(self.feature.id))
    ...
    def test_create_a_new_feature_comment(self):
        ...
        self.assertEqual(str(comment_object.picture),
    ...
    def test_create_upvote_object_and_send_one_upvote_to_feature_object(self):
        ...
        upvote_object = get_object_or_404(
            UpvoteFeature, upvoted_feature=self.feature)
        feature_point = get_object_or_404(
            Features, name=upvote_object.upvoted_feature)
